...
public class Tensorflow2DEvaluatorTest {
    private static final ClassLoader LOADER = Tensorflow2DEvaluatorTest.class.getClassLoader();
    ...
    public void evaluate_table() throws EvaluationException {
        ...
        assertEquals(-0.17967770993709564, ((float[]) tensor.getData())[0], 0.000001);
        assertEquals(-0.7580751776695251, ((float[]) tensor.getData())[1], 0.000001);
    }

    @Test
    public void evaluate_tensor() throws EvaluationException {

        final TensorIO tensorIO = new InputStreamJsonIntoTensorIO(MAPPER.getObjectMapper())
            .build(LOADER.getResourceAsStream("tensorflow/2d_savedmodel/batch_tensor.json"));

        TensorIO output = tensorFlowEvaluator.evaluate(tensorIO, new EvaluationContext());
        float[] outputTensor = (float[]) output.getTensor("scaled_imputed_label_predicted").getData();

        assertEquals(-0.17967776954174042, outputTensor[0], 0.000001);
        assertEquals(-0.7580751776695251, outputTensor[1], 0.000001);
    }
